...
unsigned long lastMqttResponseTime = 0; // 最后一次 MQTT 响应时间
...
bool printerOffline = false; // 打印机是否离线
...
void setup() {
  ...
  for (int i = 0; i < LED_COUNT; i++) {
    ...
    if (!strip.getPixelColor(i)) {
      Serial.println("LED 初始化失败，索引：" + String(i));
      ledOk = false;
    }
  }
  if (!ledOk) {
    Serial.println("LED 灯带初始化失败，继续运行");
  } else {
  ...
  if (currentState != AP_MODE && WiFi.status() == WL_CONNECTED && isConfigValid()) {
    if (!mqttClient.connected()) {
      ...
      if (currentState == CONNECTED_PRINTER && currentMillis - lastMqttResponseTime > mqttResponseTimeout) {
        Serial.println("MQTT 响应超时，打印机可能离线，重新连接");
        printerOffline = true;
        currentState = CONNECTING_PRINTER;
        lastModeJudgmentTime = millis();
...
void reconnectMQTT() {
  ...
  if (mqttClient.connect(clientID.c_str(), uid, accessToken)) {
    ...
    currentState = CONNECTING_PRINTER; // 保持 CONNECTING_PRINTER 直到收到有效响应
    lastModeJudgmentTime = millis();
    mqttError = "";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (!doc.containsKey("print") || doc["print"].isNull()) {
    Serial.println("MQTT 数据无效，打印机可能离线");
    printerOffline = true;
    return;
  }
  ...
  lastModeJudgmentTime = millis();
  Serial.println("打印状态：" + gcodeState + "，进度：" + String(printPercent) + "%");
}
...
void handleRoot() {
  ...
  switch (currentState) {
    ...
    case CONNECTED_PRINTER: stateText = printerOffline ? "打印机离线" : "打印机已连接"; break;
...
void handleLog() {
  ...
  String logContent = "WiFi 状态：" + String(WiFi.status() == WL_CONNECTED ? "已连接" : "未连接") + "<br>"
                      ...
                      "打印机状态：" + (printerOffline ? "离线" : gcodeState) + "<br>"
